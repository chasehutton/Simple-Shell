...
/*
static Token *make_error_token(char *s) {
   Token *token = calloc(1, sizeof(Token));
   if (!token) err(EX_OSERR, "Memory Allocation Failed.\n");
   token->type = TOKEN_ERROR;
   token->start = s;
   token->length = strlen(s);
   token->column = g_column;
   return token;
}
*/
...
/*
static Token *sq_string() {
   while (peek() != '\'' && !is_at_end()) advance();
   if (is_at_end()) {
      char *msg = malloc(1024);
      if (!msg) err(EX_OSERR, "Memory Allocation Failed.\n");
      snprintf(msg, 1024, "Undetermined String at column %d", g_column);
